	...
	<table class="table">
		...
		<tbody>
			...
			<c:forEach items="${feedbackList}" var="feedback">
				<tr>
					<th scope="row"><%=luc ++%></th>
					<td class="highlight">${feedback.getUser().getTenDangNhap()}</td>
					<td class="highlight">${feedback.getTieuDe()}</td>
					<td>
						<form action="docfeedback" method="GET" style="display: inline;">
							<input type="hidden" name="noiDung"
								value="${feedback.getNoiDung()}" /> <input type="hidden"
								name="sanpham" value="${feedback.getSanPham().getId()}" />
							<button type="submit" class="highlight1">Đọc</button>
						</form>
					</td>
					<td>
						<form action="xoafeedback" method="GET" style="display: inline;">
							<input type="hidden" name="sanPhamId" value="${feedback.getSanPham().getId()}" />
							<input type="hidden" name="userId" value="${feedback.getUser().getUser_id()}" />
